import { rehypeHighlight, parseMarkdown } from '@nuxtjs/mdc/runtime'
  ...
  const highlighter = async (code, lang, theme, options = {}) => {
    ...
    if (
      ...
      if (bundledLangs[lang]) {
        ...
        if (import.meta.dev) {
          console.warn(
            `[@nuxtjs/mdc] Language "${lang}" is not loaded to the Shiki highlighter, fallback to plain text. Add the language to "mdc.highlight.langs" to fix this.`,
          )
        }
        lang = 'text'
      }
    ...
    for (const [color, theme2] of Object.entries(themesObject)) {
      if (
        typeof theme2 === 'string' &&
        !loadedThemes.includes(theme2) &&
        !isSpecialTheme(theme2)
      ) {
        if (bundledThemes[theme2]) {
          ...
          if (import.meta.dev) {
            console.warn(
              `[@nuxtjs/mdc] Theme "${theme2}" is not loaded to the Shiki highlighter. Add the theme to "mdc.highlight.themes" to fix this.`,
            )
          }
    ...
    const transformers = [...baseTransformers]
    for (const config of await getConfigs()) {
      const newTransformers =
        typeof config.shiki?.transformers === 'function'
...
const bundledThemes = {
  'material-theme-lighter': () =>
    import('shiki/themes/material-theme-lighter.mjs').then((r) => r.default),
  'material-theme-palenight': () =>
    import('shiki/themes/material-theme-palenight.mjs').then((r) => r.default),
}
const options = {
  theme: {
